...
for alpha in alphas:
    ...
    for run in range(n_runs):
        ...
        std_reg_exp = GaussianProcessRegressor(kernel = kernel, n_restarts_optimizer=10, alpha=2*noise_scale**2).fit(X_train_exp, Y_train_exp)
...
plt.suptitle("Distributions Before and After Fairness Correction (gp base regressor)", fontsize=16, y=1.02)
plt.tight_layout()
plt.show()
